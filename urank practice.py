...
def compute_ranks(graph):
    ...
    ranks = {}
    npages = len(graph)
    for page in graph: #initializes the ranks to all be equal at first 
        ranks[page] = 1.0 / npages
    
    for i in range(0, numloops):
        newranks = {}
        for page in graph:
            newrank = (1 - d) / npages # a constant
            for node in graph:
             
                if page in graph [node]: #see if key is value in any other key
                    newrank = newrank + d*(ranks[node]/(len(graph[node])))
                    #if so then we add the rank of node/#links in that node
